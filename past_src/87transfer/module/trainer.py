...
class Trainer():
    def __init__(self, args):
        ...
        # self.criterion_domain = torch.nn.NLLLoss()
    ...
    def train_one_epoch(self, epoch):
        ...
        for i in pbar:
            ...
            if self.cert:
                chunk = self.model.one_seq_chunk()
                loss_label, err = robust_loss(chunk, 0.05, simg, slabel)
            else:
    ...
    def attack(self):
        RES = []
        print(len(self.sourcetestloader))
        print(len(self.targettestloader))
        for loader in [self.sourcetestloader, self.targettestloader]:
            results = []
